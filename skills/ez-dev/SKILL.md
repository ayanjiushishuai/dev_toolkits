---
name: ez-dev
description: "完整开发流程skill，串联需求分析→独立Review→设计→独立Review→计划→独立Review→TDD开发→独立Review→验证→完成。触发条件：用户提出"帮我开发..."、"实现一个..."、"添加...功能"、"修复...bug"、"新功能"、"特性实现"等明确开发请求时。**核心原则**：前期充分高沟通+严格独立Review发现问题，Review失败后**自纠闭环不等待确认**，中后期LLM自主决策，通过测试验证。**需要用户确认的节点**：Phase 1 方案选择、Phase 2 设计敲定、Phase 6 完成最终确认。Phase 6开发完成后自动整合文档，由用户手动merge。"
---
...
| Phase 5.1 | 用例生成 | plan.md、design.md、worktree 代码 | ✅ 独立 Subagent |
...
- 自纠后 Review 仍然失败 → 继续自纠，直到通过（最多 3 次循环）
...
**目的:** 通过独立 Subagent 生成测试用例，验证功能完整性
...
启动独立 Subagent（Agent tool）：
- 角色：测试质量审计员
- 只读取：.dev_doc/<feature>/plan.md、design.md、tests/ 目录下的测试用例
- 不加载：Phase 1-5.2 的对话上下文
- 任务：
  1. 审查用例覆盖率是否充分（每个功能点是否有对应用例）
  2. 检查边界条件和异常场景是否覆盖
  3. 评估测试用例的独立性和可重复性
  4. 识别可能遗漏的测试场景
- 输出：测试用例质量审查报告
...
| Phase 4 | 独立 Subagent（可选多并发） | **策略A: 并发 / 策略B: 串行** |
